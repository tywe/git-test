...
public class CalendarTest {
	...
	public static void calendarBasicTest(){
		...
		System.out.println("year:" + calendar.get(Calendar.YEAR) + ", month:" + calendar.get(Calendar.MONTH) + ", day:"+ calendar.get(Calendar.DAY_OF_MONTH) + ", time:"+calendar.get(Calendar.HOUR)+":"+calendar.get(Calendar.MINUTE));
	...
	public static List<Date> completeDateRange(List<Date> dateList, int timeSpan){
		...
		if(timeSpan == 0){//按天补齐
			...
			while(lastIdx < size){				
				...
				if(lcr.after(fcr) && lcr.get(Calendar.YEAR) == fcr.get(Calendar.YEAR)&& lcr.get(Calendar.MONTH) == fcr.get(Calendar.MONTH) && lcr.get(Calendar.DAY_OF_MONTH) - fcr.get(Calendar.DAY_OF_MONTH) == 1){
					firstIdx++;
					lastIdx++;
					...
					completeDateList.add(curDate);
					fcr.set(Calendar.DAY_OF_MONTH, fcr.get(Calendar.DAY_OF_MONTH) + 1);
					curDate = fcr.getTime();
					continue;
				}
			}
			...
			while(lastIdx < size){
				...
				while(lastIdx < size){				
					fcr.setTime(curDate);
					lcr.setTime(dateList.get(lastIdx));
					if((lcr.get(Calendar.HOUR_OF_DAY) - fcr.get(Calendar.HOUR_OF_DAY)) <= 1){
						firstIdx++;
						lastIdx++;
						completeDateList.add(curDate);
						continue;
						...
						completeDateList.add(curDate);
						fcr.set(Calendar.HOUR_OF_DAY, fcr.get(Calendar.HOUR_OF_DAY) + 1);
						curDate = fcr.getTime();
						continue;
					}
				}
	...
	public static void isDateEquals(){
		try {
			...
			if(date1.equals(date2))
				...
				System.out.println("date1:" + date1 + " not equals date2:" + date2);
